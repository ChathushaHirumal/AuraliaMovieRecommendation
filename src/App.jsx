...
export default function App() {
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "sans-serif" }}>
      ...
      <div>
        <button type="button" onClick={() => handleSort("rating")}>Sort by Rating</button>
        <button type="button" onClick={() => handleSort("year")}>Sort by Year</button>
        <button type="button" onClick={() => handleFilter("Action")}>Filter: Action</button>
        <button type="button" onClick={() => handleFilter("Drama")}>Filter: Drama</button>
        <button type="button" onClick={() => loadMovies()}>Reset</button>
        <button type="button" onClick={async () => setMovies(await getTopKByRatingHeap(10))}>Top 10 (Heap)</button>
        <span style={{ position: "relative", display: "inline-block" }}>
          <button type="button" onClick={handleShowHistoryDropdown}>
        ...
        <button type="button" onClick={handleRecs}>Recommend for You</button>
        <button type="button" onClick={handleRecsDiverse}>Recommend (Diverse)</button>
        <button type="button" onClick={() => setAdding(v => !v)}>
          {adding ? "Close Add Movie" : "Add Movie"}
        </button>
      ...
      </div>
      {
        adding && (
          <div style={{ margin: "16px 0", padding: 12, border: "1px solid #ddd", borderRadius: 8 }}>
            <AddMovie onAdded={() => { loadMovies(); setAdding(false); }} />
          </div>
        )
      }
